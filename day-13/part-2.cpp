...
{   
    ...
    for (; ;)
    {
        ...
        for (auto& cart : carts)
        {
            ...
            for (auto& cart : carts)
            {
                if (map.count(cart.getCoords()) != 0)
                {
                    crashIndexes.insert(i);
                    crashIndexes.insert(map[cart.getCoords()]);
                }
                else {
                    map[cart.getCoords()] = i;
                }
                i++;
            }
        }
        ...
        {
            if (crashIndexes.count(i) != 0)
            {
                continue;
            }
            newCarts.push_back(carts[i]);
        }
